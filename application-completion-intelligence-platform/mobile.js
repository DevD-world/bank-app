...
const apiFetch = window.completionIqFetch || ((input, options) => fetch(input, options));
...
async function loadState() {
  ...
  try {
    const response = await apiFetch(`${API_BASE}/api/state`, { cache: "no-store" });
...
async function deleteFromApi(path) {
  const response = await apiFetch(`${API_BASE}${path}`, { method: "DELETE" });
...
async function updateApplicationFromForm(event) {
  ...
  try {
    const response = await apiFetch(`${API_BASE}/api/applications/${encodeURIComponent(app.id)}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
async function submitManualCorrection(form) {
  ...
  const payload = await apiFetch(`${API_BASE}${path}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
async function syncOfflineQueue() {
  ...
  for (const item of queue) {
    try {
      const response = await apiFetch(`${API_BASE}/api/scan-jobs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
